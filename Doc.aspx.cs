...
public partial class Report : System.Web.UI.Page
{
    ...
    protected void bSave_Click(object sender, EventArgs e)
    {
        if (cbConfirm.Checked)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT id FROM addst WHERE name='" + ddlName.Text + "'", cn);
            DataSet ds = new DataSet();
            da.Fill(ds);
            if (fuReport.HasFile)
            {
                ...
                SqlCommand cmd3 = new SqlCommand("UPDATE addst SET reportext= '" + fileext +"' WHERE name='" + ddlName.Text + "'", cn);
                cn.Open();
                cmd3.ExecuteNonQuery();
                cn.Close();
                string filename = Path.GetFileName(fuReport.FileName);
                fuReport.SaveAs(Server.MapPath("~/" + "Reports/" + ds.Tables[0].Rows[0]["id"].ToString() + fileext));
            }
            if (fuCode.HasFile)
            {
                string fileext = Path.GetExtension(fuCode.PostedFile.FileName);
                SqlCommand cmd3 = new SqlCommand("UPDATE addst SET codeext= '" + fileext + "' WHERE name='" + ddlName.Text + "'", cn);
                cn.Open();
                cmd3.ExecuteNonQuery();
                cn.Close();
                string filename = Path.GetFileName(fuCode.FileName);
                fuCode.SaveAs(Server.MapPath("~/" + "Codes/" + ds.Tables[0].Rows[0]["id"].ToString() + fileext));
            }
            if (fuCerti.HasFile)
            {
                ...
                fuCerti.SaveAs(Server.MapPath("~/" + "Certificates/" + ds.Tables[0].Rows[0]["id"].ToString() + fileext));
            }
